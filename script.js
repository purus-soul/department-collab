...
import { getFirestore, doc, setDoc, getDoc, collection, addDoc, getDocs, updateDoc, arrayUnion, arrayRemove } from "https://www.gstatic.com/firebasejs/10.12.0/firebase-firestore.js";
...
async function renderPosts(user) {
  ...
  if (snapshot.empty) {
    postsList.innerHTML = "<div class='muted'>No projects yet. Be the first to create one!</div>";
    return;
  }
  ...
  snapshot.forEach((docSnap) => {
    ...
    div.innerHTML = `
      ...
      <div style="margin-top:10px; display:flex; align-items:center; gap:12px;">
        <button class="small-btn join-btn" ${!user ? "disabled" : ""}>${isJoined ? "✅ Joined" : "Join Project"}</button>
...
async function renderProfile(user) {
  ...
  if (myPosts.length === 0) profilePosts.innerHTML = "<div class='muted'>You have not created any projects yet.</div>";
    ...
    d.innerHTML = `<strong>${escapeHtml(p.title)}</strong><div class="muted">${escapeHtml(p.description || "")}</div>`;
